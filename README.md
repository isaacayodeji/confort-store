...
1. Import Dependencies:
   ...
   - Import `formatPrice`, `customFetch`, and `useState` from `'../utils/index'`.
   - Import `Link` from `'react-router-dom'`.
...
1. **Initialize Necessary Imports**:
   ...
   - Import utility functions `formatPrice` and `generateAmountOptions` from the `../utils/index` directory.
